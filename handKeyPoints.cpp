...
float transformNetInput(torch::Tensor& inputTensor, const cv::Mat& src_img, int tensor_index)
{
	// lmbada expression
...
void detectBbox(std::vector<cv::Rect>& handrect, torch::jit::script::Module& model, const cv::Mat& inputImage)
{
	...
	for (auto iter = locations.begin(); iter != locations.end(); iter++) {
		...
		if (pixelcount > 0) {
			...
			l_t.x = std::max(l_t.x, 0.f); l_t.y = std::max(l_t.y, 0.f);
			r_b.x = std::min(r_b.x, inputImage.cols - 1.f); r_b.y = std::min(r_b.y, inputImage.rows - 1.f);
...
void detecthand(std::vector<std::map<float, cv::Point2f>>& manypoints,
	...
	const cv::Mat& inputImage, const std::vector<cv::Rect>& handrect)
{
